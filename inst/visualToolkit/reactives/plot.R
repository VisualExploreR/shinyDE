...
scatterPlotInputs <- reactive({
  list(x=input$x, y=y(), color=color(), colorAsFactor=colorAsFactor(), treatAsFacVarCol=treatAsFacVarCol(), 
...
linePlotInputs <- reactive({
  list(x=input$x, y=y(), color=color(), colorAsFactor=colorAsFactor(), alpha=alpha())
})
...
barPlotInputs <- reactive({
  list(x=input$x, y=y(), fill=fill(), fillAsFactor=fillAsFactor(), position=position(), alpha=alpha())
})

## histogram inputs
histogramInputs <- reactive({
  list(x=input$x, fill=fill(), fillAsFactor=fillAsFactor(), position=position(), 
       binWidth=binWidth(), alpha=alpha())
})

## density plot inputs
densityPlotInputs <- reactive({
  list(x=input$x, fill=fill(), fillAsFactor=fillAsFactor(), 
       densBlkLineCond=densBlkLineCond(), alpha=alpha())
})

## box plot inputs
boxPlotInputs <- reactive({
  list(x=input$x, y=y(), fill=fill(), fillAsFactor=fillAsFactor(), alpha=alpha())
})

## path plot inputs
pathPlotInputs <- reactive({
  list(x=input$x, y=y(), alpha=alpha())
})
