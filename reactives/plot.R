...
# 4. coord_fip option
# 5. Y-option disapear from histogram
...
# 7. geom_smooth: no smoothing, loess, and linear



## COMPLETED ITEMS
# 3. shapes, sizes, alpha, jitter, eom_smooth (with lowess and linear options) 
...
plotInput <- reactive({
  ...
  ## don't plot anything if any of the control pieces are missing (i.e. not loaded)
  ...
  if (is.null(alpha) | is.null(shape) | is.null(size)) {return(NULL)}
